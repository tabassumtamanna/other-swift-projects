...
class TMDBClient {
    ...
    static let apiKey = "7a541f62737cbdb9fa2c6fb6353e6077"
    //static let apiKey = ""
    ...
    class func taskForPOSTRequest<RequestType: Encodable, ResponseType: Decodable>(url: URL, responseType: ResponseType.Type, body: RequestType, completion: @escaping (ResponseType?, Error?) -> Void){
        ...
        let task = URLSession.shared.dataTask(with: request)  {(data, response, error) in
            ...
            do {
                let requestObject = try docoder.decode(ResponseType.self, from: data)
                DispatchQueue.main.async {
                ...
                do {
                    let errorResponse = try docoder.decode(TMDBResponse.self, from: data) as Error
                    DispatchQueue.main.async {
    ...
    class func markFavorite(movieId: Int, favorite: Bool, completion: @escaping (Bool, Error?) -> Void){
        ...
        taskForPOSTRequest(url: Endpoints.markFavoritelist.url, responseType: TMDBResponse.self, body: body){ (response, error) in
